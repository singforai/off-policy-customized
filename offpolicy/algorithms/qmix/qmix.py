...
class QMix(Trainer):
    ...
    def train_policy_on_batch(self, batch, update_policy_id=None):
        ...
        Q_tot_seq = torch.reshape(self.mixer(agent_q_seq, cent_obs_batch[:-1]),(60,32,1))#.squeeze(-1)
        next_step_Q_tot_seq = torch.reshape(self.target_mixer(agent_nq_seq, cent_obs_batch[1:]),(60,32,1))#.squeeze(-1)
